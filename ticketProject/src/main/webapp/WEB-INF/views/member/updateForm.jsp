...
<body>
	<div id="update_box">
		<h2>Edit Profile</h2>
		<form method="post" name="m_frm" action="memberUpdate">
			<fieldset>
				...
				<c:choose>
					<c:when test="${empty dto.provider }"> 
						...
						<input type="password" name="pwdCheck" placeholder="비밀번호재확인">
					</c:when>
					<c:otherwise>
				...
				<br> <label>Name</label> <input type="text" name="name"
					value="${dto.name }"> <br> <label>Nickname</label> <input
					type="text" name="nickname" value="${ dto.nickname }"> <br>
				<label>E-Mail</label>
				<c:choose>
					<c:when test="${empty dto.provider }">
...
<%-- 				<fmt:formatDate var="birthday" value="${birth}" pattern="yyyy-MM-dd" /> --%>
				...
				<%-- 				<c:out value="${fn:substring(birth, 0, 10)}"/> --%>

				<%-- 				<fmt:parseDate value="${dto.birth}" var="birth" pattern="yyyyMMdd"/> --%>
				<%-- 				<fmt:formatDate value="${birth}" var="birthday" pattern="yyyy.MM.dd"/> --%>
				<br>
			</fieldset>
			<fieldset>
				<legend>Optional</legend>
				<label>Zip Code</label> <input type="text" id="sample6_postcode"
					name="zip_num" value="${dto.zip_num}" readonly> <input
					type="button" onclick="sample6_execDaumPostcode()" class="dup"
					value="우편번호 찾기"><br> <label>Address</label><input
					type="text" id="sample6_address" size="50" name="address1"
					value="${dto.address1}" readonly><br> <label>detailAddress</label><input
					type="text" id="sample6_detailAddress" name="address2" size="50"
					value="${dto.address2}"><br> <label>extraAddress</label><input
			...
			<div id="buttons" class="update_btn_box">
				<input type="button" value="정보수정" class="submit btn_update_confirm"
					onclick="go_update();"> <input type="reset" value="취소"
					class="cancel">
			</div>
		</form>
	</div>
